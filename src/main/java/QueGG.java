   ...
   //private static String inputDir = "result/it/";
    ...
    private static Result runParserForQald(Grammar grammar, String id, String sentence, String givenSparql) throws Exception {
        try {
            ...
            id = StringModifier.deleteQuote(id);
            sentence = StringModifier.deleteQuote(sentence);
            List<String> allSparqls = grammar.parser(sentence);
            List<String> sparqls = filterSparql(allSparqls);
            if (!sparqls.isEmpty()) {
    ...
    private static List<String> filterSparql(List<String> allSparqls) {
        List<String> filterSparqls = new ArrayList<String>();
        for (String sparql : allSparqls) {
            if (sparql.contains("?Arg")) {
                continue;
            }
            filterSparqls.add(sparql);
        }
        return filterSparqls;
    }
